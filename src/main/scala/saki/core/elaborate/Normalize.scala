...
//    }
//
//    private def withParam[R](param: Param[Term])(action: Context => R): R = {
//      action(ctx + (param.ident -> Term.Variable(param.ident)))
//    }
//  }
//  
...
//        }.tryMatchTerm(scrutineesNorm).map(_.normalize).getOrElse {
