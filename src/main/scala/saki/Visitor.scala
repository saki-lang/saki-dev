...
class Visitor extends SakiBaseVisitor[Term | Seq[Term] | Pattern] {
  ...
  override def visitPatternVariable(ctx: PatternVariableContext): Pattern = Pattern.Variable(ctx.ident.getText)
  ...
  override def visitAtomType(ctx: AtomTypeContext): Term = Term.Universe(0)
  ...
  override def visitLiteralInt(ctx: LiteralIntContext): Term = Term.PrimitiveValue(IntValue(ctx.value.getText.toInt))

  override def visitLiteralFloat(ctx: LiteralFloatContext): Term = Term.PrimitiveValue(FloatValue(ctx.value.getText.toFloat))

  override def visitLiteralBool(ctx: LiteralBoolContext): Term = Term.PrimitiveValue(BoolValue(ctx.value.getText.toBoolean))

  override def visitLiteralChar(ctx: LiteralCharContext): Term = Term.PrimitiveValue(CharValue(ctx.value.getText.charAt(1)))
